...
- ~~The four failing `VoicelyUITests` cases~~ — DONE (2026-07-07): also not stale tests — an accessibility regression. `noteLibraryList`'s `.accessibilityElement(children: .contain)` stamped `LibraryScreen` over its descendants, so `NoteLibraryList`, `RecordingControls` and `RecordingModelPickerButton` disappeared from the tree (confirmed by dumping `app.debugDescription`). Removed the container marking so inner identifiers survive; dropped two now-redundant container assertions and pointed the queued-note test at `TranscribeNowButton` (that action was renamed and re-identified). All UI tests pass.
